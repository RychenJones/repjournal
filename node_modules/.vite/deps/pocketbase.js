//#region node_modules/pocketbase/dist/pocketbase.es.mjs
...
var RealtimeService = class extends BaseService {
	constructor() {
		super(...arguments), this.clientId = "", this.eventSource = null, this.subscriptions = {}, this.lastSentSubscriptions = [], this.maxConnectTimeout = 15e3, this.reconnectAttempts = 0, this.maxReconnectAttempts = 1 / 0, this.predefinedReconnectIntervals = [
			...
			1500,
			2e3
		], this.pendingConnects = [], this.pendingSubmits = [], this.isProcessingPendingSubmits = !1;
	}
	get isConnected() {
	...
	async unsubscribe(e) {
		if (e) {
			const t = this.getSubscriptionsByTopic(e);
			for (let e in t) if (this.hasSubscriptionListeners(e)) {
	...
	async unsubscribeByTopicAndListener(e, t) {
		...
		for (let e in s) {
			...
			let s = !1;
			for (let i = this.subscriptions[e].length - 1; i >= 0; i--) this.subscriptions[e][i] === t && (s = !0, delete this.subscriptions[e][i], this.subscriptions[e].splice(i, 1), this.eventSource?.removeEventListener(e, t));
			s && (this.subscriptions[e].length || delete this.subscriptions[e]);
		}
		await this.submitSubscriptions();
	}
	...
	async submitSubscriptions() {
		return new Promise(((e, t) => {
			this.pendingSubmits.push({
				resolve: e,
				reject: t
			}), 1 == this.pendingSubmits.length && queueMicrotask((() => this.finalizePendingSubscriptions()));
		}));
	}
	async finalizePendingSubscriptions() {
		if (this.isProcessingPendingSubmits || !this.pendingSubmits.length) return;
		const e = this.pendingSubmits.slice();
		...
		try {
			...
			this.isProcessingPendingSubmits = !1, this.pendingSubmits.length > 0 && await this.finalizePendingSubscriptions();
		}
	}
	...
	async sendSubscriptions() {
		if (this.clientId) {
			...
			if (this.hasUnsentSubscriptions()) return this.addAllSubscriptionListeners(), this.lastSentSubscriptions = this.getNonEmptySubscriptionKeys(), this.client.send("/api/realtime", {
				method: "POST",
				body: {
	...
	async connect() {
		if (!(this.reconnectAttempts > 0)) return new Promise(((e, t) => {
			this.pendingConnects.push({
				resolve: e,
				reject: t
			}), 1 == this.pendingConnects.length && queueMicrotask((() => this.initConnect()));
		}));
	}
	initConnect() {
		this.disconnect(!0), clearTimeout(this.connectTimeoutId), this.connectTimeoutId = setTimeout((() => {
			...
			const t = e;
			this.clientId = t?.lastEventId, this.lastSentSubscriptions = [], this.submitSubscriptions().then((async () => {
				let t = 3;
				for (; this.hasUnsentSubscriptions() && t > 0;) t--, await this.submitSubscriptions();
				...
				for (let t in s) for (let i of s[t]) i(e);
				this.hasUnsentSubscriptions() && await this.submitSubscriptions();
			})).catch(((e) => {
				this.clientId = "", this.lastSentSubscriptions = [], this.connectErrorHandler(e);
			}));
		}));
	}
	...
	disconnect(e = !1) {
		if (this.clientId && this.onDisconnect && this.onDisconnect(Object.keys(this.subscriptions)), clearTimeout(this.connectTimeoutId), clearTimeout(this.reconnectTimeoutId), this.removeAllSubscriptionListeners(), this.client.cancelRequest(this.getSubscriptionsCancelKey()), this.eventSource?.close(), this.eventSource = null, this.clientId = "", this.lastSentSubscriptions = [], !e) {
...
var CollectionService = class extends CrudService {
	...
	async getAllOAuth2Providers(e) {
		return e = Object.assign({ method: "GET" }, e), this.client.send(this.baseCrudPath + "/meta/oauth2-providers", e);
	}
	async dryRunViewQuery(e, t) {
		return t = Object.assign({
			method: "POST",
			body: { query: e }
		}, t), this.client.send(this.baseCrudPath + "/meta/dry-run-view", t);
	}
};
var LogService = class extends BaseService {
	...
	async truncate(e) {
		return e = Object.assign({ method: "DELETE" }, e), this.client.send("/api/logs", e).then((() => !0));
	}
};
...
RegExp.escape || (RegExp.escape = function(e) {
	return e.replace(/[/\-\\^$*+?.()|[\]{}]/g, "\\$&");
});
var Client = class {
	get baseUrl() {
	...
	filter(e, t) {
		...
		const i = Object.keys(s).map(((e) => RegExp.escape(e))).join("|");
		if (!i) return e;
		return e.replace(new RegExp(i, "g"), ((e) => function stringify(e) {
			switch (typeof e) {
				case "boolean":
				...
				default:
					...
					if (e instanceof Date) return JSON.stringify(e.toISOString().replace("T", " "));
					const t = JSON.stringify(e);
					return t.startsWith("[") || t.startsWith("{") ? JSON.stringify(t) : t;
			}
		}(s[e])));
	}
	getFileUrl(e, t, s = {}) {
